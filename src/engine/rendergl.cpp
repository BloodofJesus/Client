...
void crosshairbump()
{
	if(!GraphOX::_Graphox_crosshairbump) return;
...
void drawcrosshair(int w, int h)
{
    ...
    else
    {
		...
		if(GraphOX::_Graphox_crosshairbump) index = 0;
...
FVARP(conscale, 1e-3f, 0.33f, 1e3f);VARP(showdetailedhud, 0, 1, 1);
...
void gl_drawhud(int w, int h)
{
    ...
    int contw = int(w/conscale), conth = int(h/conscale), abovehud2 = conth - FONTH, limitgui2;
    ...
    if(!hidehud && !mainmenu)
    {
        if(GraphOX::_Graphox_hudtheme == 0)
	    {
		if(!hidestats)
		{
		    glPushMatrix();
			...
			if(gameinfo)
			{
			    draw_text(gameinfo, conw-max(5*FONTH, 2*FONTH+text_width(gameinfo)), conth-FONTH*5/2-roffset);
			    DELETEA(gameinfo);
			    roffset += FONTH;
			}
		    }
	    ...
	    else if(GraphOX::_Graphox_hudtheme == 1)
	    {
	    if(!hidestats)
	    {
		glPushMatrix();
