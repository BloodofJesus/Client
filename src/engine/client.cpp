...
ICOMMAND(rate, "", (), conoutf("\f3WARNING: \f2Command \"rate\" has been disabled"));
...
void connectserv(const char *servername, int serverport, const char *serverpassword)
{
    ...
    if(servername)
    {
        ...
        defformatstring(text)("attempting to connect to %s:%d", servername, serverport);
	conoutf(0, text);
    ...
    else
    {
        ...
        conoutf(0, "attempting to connect over LAN");
...
void disconnect(bool async, bool cleanup)
{
	...
	stopsounds(); // stop sounds while disconnecting
}
...
void trydisconnect()
{
    ...
    {
        conoutf(0, "attempting to disconnect...");
...
void gets2c()           // get updates from the server
{
    ...
    {
        conoutf(0, "attempting to connect...");
        connmillis = totalmillis;
        ++connattempts;
        if(connattempts > 3)
        {
    ...
    switch(event.type)
    {
        ...
        case ENET_EVENT_TYPE_RECEIVE:
            if(discmillis) conoutf(0,"attempting to disconnect...");
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
            ...
            {
                conoutf(0, "\f3could not connect to server");
                abortconnect();
            }
            else
            {
				defformatstring(text)("\f3server network error, disconnecting - %s", disc_reasons[event.data]);
                if(!discmillis || event.data) conoutf(0, text);
                disconnect();
            }
            return;
