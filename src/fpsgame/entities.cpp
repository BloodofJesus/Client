...
namespace entities
{
    ...
    void pickupeffects(int n, fpsent *d)
    {
        ...
        if(d==player1)
        {
            if(GraphOX::_Graphox_crosshairbump && GraphOX::_Graphox_bumpcrossonpickup) crosshairbump();
            switch(type)
            {
                case I_SHELLS: case I_BULLETS: case I_ROCKETS: case I_ROUNDS: case I_GRENADES: case I_CARTRIDGES:
    ...
    void teleport(int n, fpsent *d)     // also used by monsters
    {
        ...
        for(;;)
        {
            ...
            if(ents[e]->attr2==tag)
            {
                ...
                if(d == player1)
                    if(GraphOX::_Graphox_crosshairbump && GraphOX::_Graphox_bumpcrossonteleport) crosshairbump();
                break;
            }
        }
    }
