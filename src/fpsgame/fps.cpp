...
//    	}
    ...
    void statsacc() //accuracy stats updating
    {
        ...
        if((totalmillis >= lastacc+30000 && firstcheck) || (totalmillis >= lastacc+5000 && !firstcheck))  //First check - 30s to prevent getting 100% accuracy into avg
        {
            ...
            else if(stats[3] && accuracy && accuracy != oldacc)
            {
                stats[3] = ((stats[3]*stats[11])+accuracy) / (stats[11]+1);
                stats[11]++;
                oldacc = accuracy;
            }
            lastacc = totalmillis;
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        else if(isteam(d->team, actor->team))
        {
            ...
            if(actor==player1)
            {
                ...
                if(GraphOX::_Graphox_autosorry == 1)
            	{
            	    GraphOX::Auto(sr_string, dname);
                }
        ...
        else
        {
            ...
            else
            {
                ...
                if(actor==player1)
                {
                    if(GraphOX::_Graphox_crosshairbump && GraphOX::_Graphox_bumpcrossonkill) crosshairbump();
    ...
    {
        if(GraphOX::_Graphox_hudtheme == 1)
    ...
    {
        if(GraphOX::_Graphox_hudtheme == 1) return;
    ...
    {
	if(GraphOX::_Graphox_hudtheme == 1) GraphOX::grahud(w, h);
