    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_TEXT:
            {
                ...
                if(GraphOX::isignored(d->clientnum)) break;
            ...
            case N_SAYTEAM:
            {
                ...
                getstring(text, p);
                filtertext(text, text);
                if(!t || GraphOX::isignored(t->clientnum)) break;
            ...
            case N_INITCLIENT:            // another client either connected or changed name/team
            {
                ...
                {
                    if(strcmp(d->name, text) && !GraphOX::isignored(d->clientnum))
            ...
            case N_SWITCHNAME:
                ...
                if(d)
                {
                    ...
                    {
                        if(!GraphOX::isignored(d->clientnum)) conoutf("%s changed name to %s", colorname(d), colorname(d, text));
                        copystring(d->name, text, MAXNAMELEN+1);
                    }
                }
                break;
